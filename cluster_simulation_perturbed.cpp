...
int main(){
	...
	int N_maxpertstep = 10;//this, is the number of perturbation steps performed per attempt, and can ofcourse be varied with respect to cluster size
	int xpstep = 1; int ypstep = 1; int zpstep = 1;//perturbation steps in all three directions, can also be scaled wrt the size of the cluster
	double perturb_P = 0.5;//probability for attempting a perturbation at all
	double t = 0.5;//base of instability probability
	int Nmaxneighbor = 13;//number of neighbors that can effectively define a hole, in Oh geometery this is 13 considering all diagonal neighbors
	//shape parameters
	...
	while(Nparticle_current < (Nparticle_max + 1)){
		...
		if( islessequal(tossacoin,P) ){
			...
			if( (newcluster.is_colliding(temporary_particle, bondlength)) != false ){
				...
				double s = pow(t,(Nmaxneighbor-N_neighbors));
				...
				if( islessequal(tossacoin,s) ){
					...
					//attempt a cluster perturbation
					cluster pertclust; pertclust = perturb(newcluster, N_maxpertstep, xpstep, ypstep, zpstep, perturb_P);
					newcluster = pertclust;
					//no particles were inserted so we will not increase the particle number
